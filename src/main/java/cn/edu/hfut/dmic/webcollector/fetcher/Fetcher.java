...
public class Fetcher {
	...
	public static class FetchQueue {
		...
		public final List<FetchItem> queue = Collections
				...
				while (running) {
					try {
						...
						try {
							...
							if (crawlDatum.getStatus() == CrawlDatum.STATUS_DB_SUCCESS
									&& !next.isEmpty()) {
						...
						/*
						 * 老代码 if (visit(crawlDatum, page, next)) { try {
						 * dbManager.wrtieFetchSegment(crawlDatum); if
						 * (page.getResponse() == null) { continue; } if
						 * (page.getResponse().isRedirect()) { if
						 * (page.getResponse().getRealUrl() != null) {
						 * dbManager.writeRedirectSegment(crawlDatum,
						 * page.getResponse().getRealUrl().toString()); } } if
						 * (!next.isEmpty()) {
						 * dbManager.wrtieParseSegment(next); }
						 * 
						 * } catch (Exception ex) {
						 * LOG.info("Exception when updating db", ex); } } if
						 * (visitInterval > 0) { try {
						 * Thread.sleep(visitInterval); } catch (Exception
						 * sleepEx) { } }
						 */
					} catch (Exception ex) {
		...
		try {
			...
			do {
				...
				if ((System.currentTimeMillis() - lastRequestStart.get()) > Config.THREAD_KILLER) {
					LOG.info("Aborting with " + activeThreads
							+ " hung threads.");
					break;
				}

			} while (running
					&& (startedThreads.get() != threads || activeThreads.get() > 0));
			running = false;
	...
	/*
	 * public Requester getRequester() { return requester; } public void
	 * setRequester(Requester requester) { this.requester = requester; } public
	 * boolean visit(CrawlDatum crawlDatum, Page page, CrawlDatums next) {
	 * String url = crawlDatum.getUrl(); if (page.getStatus() ==
	 * Page.STATUS_FETCH_SUCCESS) {
	 * crawlDatum.setStatus(CrawlDatum.STATUS_DB_FETCHED);
	 * crawlDatum.setHttpCode(page.getResponse().getCode()); if
	 * (!page.getResponse().isNotFound()) { try { visitor.visit(page, next); }
	 * catch (Exception ex) { LOG.info("Exception when visit URL: " + url, ex);
	 * return false; } } else { try { visitor.notFound(page, next); } catch
	 * (Exception ex) { LOG.info("Exception when not found URL: " + url, ex);
	 * return false; } } try { visitor.afterVisit(page, next); } catch
	 * (Exception ex) { LOG.info("Exception after visit URL: " + url, ex);
	 * return false; } } else if (page.getStatus() == Page.STATUS_FETCH_FAILED)
	 * { crawlDatum.setStatus(CrawlDatum.STATUS_DB_UNFETCHED); try {
	 * visitor.fail(page, next); } catch (Exception ex) {
	 * LOG.info("Exception when execute failed URL: " + url, ex); return false;
	 * } } return true; } public Page getPage(CrawlDatum crawlDatum) { String
	 * url = crawlDatum.getUrl(); Page page; HttpResponse response = null; int
	 * retryIndex = 0; Exception lastException = null; int retryCount = 0; for
	 * (; retryIndex <= retry; retryIndex++) { try { response =
	 * requester.getResponse(crawlDatum);//this.getHttpResponse(crawlDatum);
	 * break; } catch (Exception ex) { String suffix = "th "; switch (retryIndex
	 * + 1) { case 1: suffix = "st "; break; case 2: suffix = "nd "; break; case
	 * 3: suffix = "rd "; break; default: suffix = "th "; } lastException = ex;
	 * if (retryIndex < retry) { StringBuilder sb = new StringBuilder();
	 * sb.append("retry ").append(retryIndex + 1).append(suffix).append("URL:")
	 * .append(url).append(" after ").append(retryInterval)
	 * .append("ms ").append("(").append(ex.toString()).append(")"); String
	 * logMessage = sb.toString(); LOG.info(logMessage); retryCount++; if
	 * (retryInterval > 0) { try { Thread.sleep(retryInterval); } catch
	 * (Exception sleepEx) { } } } } } if (response != null) { if
	 * (!response.isNotFound()) { LOG.info("fetch URL: " + url); } else {
	 * //404应该被当作抓取成功，因为404告诉爬虫页面不存在，以后不需要重试页面 LOG.info("ignore URL: " + url +
	 * " (not found)"); } page = Page.createSuccessPage(crawlDatum, retryCount,
	 * response); } else { LOG.info("failed URL: " + url + " (" + lastException
	 * + ")"); page = Page.createFailedPage(crawlDatum, retryCount,
	 * lastException); } return page; } public long getRetryInterval() { return
	 * retryInterval; } public void setRetryInterval(long retryInterval) {
	 * this.retryInterval = retryInterval; } public long getVisitInterval() {
	 * return visitInterval; } public void setVisitInterval(long visitInterval)
